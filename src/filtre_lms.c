...
#define LMS_STEP_SIZE 0.01   // Taille de pas pour le LMS
#define LMS_FILTER_ORDER 32  // Ordre du filtre LMS
...
// Initialiser le filtre LMS
void initLMSFilter(LMSFilter *filter) {
...
// Mettre à jour le filtre LMS
float updateLMSFilter(LMSFilter *filter, float input, float desired) {
    float output = 0.0f;

    // Calcul de la sortie
    for (int i = LMS_FILTER_ORDER - 1; i > 0; i--) {
        filter->history[i] = filter->history[i - 1];
        output += filter->weights[i] * filter->history[i];
    }
    filter->history[0] = input;
    output += filter->weights[0] * filter->history[0];
...
// Générer le signal anti-bruit avec un décalage de phase
...
static int audioCallback(const void *inputBuffer, void *outputBuffer,
    ...
    static LMSFilter lmsFilter;
    static int initialized = 0;

    if (!initialized) {
        initLMSFilter(&lmsFilter);
        initialized = 1;
    }
    ...
    if (band->detected_frequency != -1) {
        ...
        // Ajouter le signal du filtre LMS pour un anti-bruit adaptatif
        for (unsigned long i = 0; i < framesPerBuffer; i++) {
            float desired = outBuffer[i];
            outBuffer[i] = updateLMSFilter(&lmsFilter, inBuffer[i], desired);
        }
    } else {
        for (unsigned long i = 0; i < framesPerBuffer; i++) {
            outBuffer[i] = updateLMSFilter(&lmsFilter, inBuffer[i], 0.0f);
        }
    }
